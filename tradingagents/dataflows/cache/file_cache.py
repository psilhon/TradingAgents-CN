...
class StockDataCache:
    ...
    def save_stock_data(
        ...
        if self.should_skip_cache_for_content(content_to_check, "股票数据"):
            ...
            cache_key = self._generate_cache_key(
                "stock_data", symbol, start_date=start_date, end_date=end_date, source=data_source, market=market_type, skipped=True
            )
    ...
    def save_news_data(
        ...
        if self.should_skip_cache_for_content(news_data, "新闻数据"):
            # 生成一个虚拟的缓存键，但不实际保存
            cache_key = self._generate_cache_key("news", symbol, start_date=start_date, end_date=end_date, source=data_source, skipped=True)
    ...
    def save_fundamentals_data(self, symbol: str, fundamentals_data: str, data_source: str = "unknown") -> str:
        ...
        if self.should_skip_cache_for_content(fundamentals_data, "基本面数据"):
            ...
            market_type = self._determine_market_type(symbol)
            cache_key = self._generate_cache_key(
                "fundamentals", symbol, source=data_source, market=market_type, date=datetime.now().strftime("%Y-%m-%d"), skipped=True
            )
