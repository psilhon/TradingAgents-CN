...
class TradingAgentsLogger:
    ...
    def _add_file_handler(self, logger: logging.Logger):
        ...
        file_handler = logging.handlers.RotatingFileHandler(log_file, maxBytes=max_size, backupCount=backup_count, encoding="utf-8")
    ...
    def _add_error_handler(self, logger: logging.Logger):
        ...
        error_handler = logging.handlers.RotatingFileHandler(error_log_file, maxBytes=max_size, backupCount=backup_count, encoding="utf-8")
    ...
    def log_analysis_complete(
        self, logger: logging.Logger, stock_symbol: str, analysis_type: str, session_id: str, duration: float, cost: float = 0
    ):
        """记录分析完成"""
        logger.info(
    ...
    def log_module_start(self, logger: logging.Logger, module_name: str, stock_symbol: str, session_id: str, **extra_data):
        """记录模块开始分析"""
        logger.info(
    ...
    def log_module_error(
        self, logger: logging.Logger, module_name: str, stock_symbol: str, session_id: str, duration: float, error: str, **extra_data
    ):
        """记录模块分析错误"""
        logger.error(
    ...
    def log_token_usage(
        self, logger: logging.Logger, provider: str, model: str, input_tokens: int, output_tokens: int, cost: float, session_id: str
    ):
        """记录Token使用"""
        logger.info(
