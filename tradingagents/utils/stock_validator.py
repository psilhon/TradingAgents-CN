...
class StockDataPreparer:
    ...
    def prepare_stock_data(
        self, stock_code: str, market_type: str = "auto", period_days: int | None = None, analysis_date: str | None = None
    ...
    def _validate_format(self, stock_code: str, market_type: str) -> StockDataPreparationResult:
        ...
        if not stock_code:
            return StockDataPreparationResult(
                is_valid=False, stock_code=stock_code, error_message="股票代码不能为空", suggestion="请输入有效的股票代码"
            )

        if len(stock_code) > 10:
            return StockDataPreparationResult(
                is_valid=False, stock_code=stock_code, error_message="股票代码长度不能超过10个字符", suggestion="请检查股票代码格式"
            )
    ...
    def _prepare_china_stock_data(self, stock_code: str, period_days: int, analysis_date: str) -> StockDataPreparationResult:
    ...
    async def _prepare_china_stock_data_async(self, stock_code: str, period_days: int, analysis_date: str) -> StockDataPreparationResult:
        ...
        try:
            ...
            if historical_data and "❌" not in historical_data and "获取失败" not in historical_data:
                ...
                has_valid_data = len(historical_data) > 50 and any(indicator in historical_data for indicator in data_indicators)
    ...
    def _check_database_data(self, stock_code: str, start_date: str, end_date: str) -> dict:
        ...
        try:
            ...
            if not adapter.use_app_cache or adapter.db is None:
                return {"has_data": False, "is_latest": False, "record_count": 0, "latest_date": None, "message": "MongoDB缓存未启用"}

            # 查询数据库中的历史数据
            df = adapter.get_historical_data(stock_code, start_date, end_date)

            if df is None or df.empty:
                return {"has_data": False, "is_latest": False, "record_count": 0, "latest_date": None, "message": "数据库中没有数据"}
            ...
            return {"has_data": False, "is_latest": False, "record_count": 0, "latest_date": None, "message": f"检查失败: {e!s}"}
    ...
    def _prepare_hk_stock_data(self, stock_code: str, period_days: int, analysis_date: str) -> StockDataPreparationResult:
        """预获取港股数据"""
    ...
    def _prepare_us_stock_data(self, stock_code: str, period_days: int, analysis_date: str) -> StockDataPreparationResult:
        """预获取美股数据"""
