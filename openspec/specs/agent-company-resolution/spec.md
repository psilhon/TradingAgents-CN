# agent-company-resolution Specification

## Purpose

锁定 agent 层「由股票代码解析人类可读公司名」的能力契约：单一入口 + 跨市场（A 股 / 港股 / 美股）解析行为 + 失败降级规则。此前该逻辑以 7 份近乎相同的 `_get_company_name` 拷贝散落在 analysts / researchers，本 capability 收敛到 `tradingagents/agents/utils/company_resolver.py`，禁止再 fork 成多份。
