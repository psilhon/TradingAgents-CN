...
- **当前版本**：`v1.2.0`（fork minor release；`pyproject.toml` 已对齐；上游 `v1.0.1`）
- **当前阶段**：**v1.2.0 已发布** — 用户工作流稳定性 + 配置层 truth source 收敛（MongoDB 唯一可写源 / JSON 退化为兼容快照）+ DeepSeek V4 + 视觉重构延伸。OpenSpec 累计 13 条 changes archived（含本轮 `stabilize-user-workflows`）。第三梯队剩 3 条（cache 层 / app 反向 import / agent state）+ 第四梯队 3 条按需排期，见 `docs/code-review-2026-05-05.md`。
...
| 文件 | 用途 | tracked |
|------|------|------|
| `docker-compose.yml` | 本地 dev / 改代码后 build；image `tradingagents-{backend,frontend}:v1.2.0` | ✅ |
