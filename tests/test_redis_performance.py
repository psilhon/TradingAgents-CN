...
class RedisPerformanceTester:
    ...
    def test_concurrent_connections(self, num_threads=10, operations_per_thread=100):
        ...
        def worker_task(thread_id):
            ...
            try:
                # 每个线程创建自己的Redis连接
                client = redis.Redis(host=self.host, port=self.port, password=self.password, db=self.db, decode_responses=True)
                ...
                return {"thread_id": thread_id, "error": str(e), "successful_operations": 0, "failed_operations": operations_per_thread * 2}
...
def main():
    ...
    parser.add_argument("--test", choices=["latency", "throughput", "concurrent", "memory", "all"], default="all", help="测试类型")
    ...
    if args.test == "all":
        ...
        if args.test == "latency":
            ...
            results = {"set_throughput": tester.test_throughput(1000, "set"), "get_throughput": tester.test_throughput(1000, "get")}
