...
3. **改 tracked 上游文件**（pyproject.toml / .gitignore）会增加上游 sync 冲突；**优先用 `.env` / `pyproject.toml` 末尾追加 / `.git/info/exclude` 等本地化机制**
...
| 5 | **原生服务配置** | `config/mongod.conf`（`bindIp: 127.0.0.1`, `port: 54302`）<br>`config/redis.conf`（`bind 127.0.0.1`, `port 54303`）| 本机原生 mongo/redis 监听地址；由 `scripts/local-services.sh` 加载 | 改后 `./scripts/local-services.sh restart` + `just audit-binds` |
| 6 | **上游 tracked 代码 hardcode**（最低） | `app/services/config_service.py:491` `port=27017` fallback<br>`scripts/create_default_admin.py` `localhost:27017` | 上游默认值；fork 通过上面 5 层之一覆盖；不直接改（除非在 OpenSpec change 范围）| 改 fork-local 配置后必跑 `just audit-binds` 确认覆盖生效 |
