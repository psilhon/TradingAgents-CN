...
数据源：akshare `stock_a_indicator_lg(symbol)` 返回个股历史 PE/PB 时间序列，
取最新一条 upsert 到 mongo `stock_indicators`。
...
class StockIndicatorService:
    ...
    def _fetch_latest_indicator(code: str) -> dict[str, Any] | None:
        ...
        akshare `stock_a_indicator_lg(symbol)` 返回 DataFrame 含 trade_date /
        pe / pe_ttm / pb / ps / ps_ttm / dv_ratio / dv_ttm / total_mv 等。
        取最后一行（最新日期）。
        """
        ...
        last = df.iloc[-1]
        # 兼容列名
        d_raw = last.get("trade_date") or last.get("date")
        if d_raw is None:
            return None
        return {
            "date": str(d_raw)[:10],
