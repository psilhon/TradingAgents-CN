...
class AnalysisService:
    ...
    async def _execute_single_analysis_async(self, task: AnalysisTask):
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                result = await loop.run_in_executor(executor, self._execute_analysis_sync_with_progress, task, progress_tracker)
    ...
    async def submit_single_analysis(self, user_id: str, request: SingleAnalysisRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            background_task = asyncio.create_task(self._execute_single_analysis_async(task))
    ...
    async def execute_analysis_task(
        self, task: AnalysisTask, progress_callback: Optional[Callable[[int, str], None]] = None
    ) -> AnalysisResult:
        """执行单个分析任务"""
        try:
            logger.info(f"开始执行分析任务: {task.task_id} - {task.symbol}")
