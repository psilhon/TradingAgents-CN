...
from app.routers import websocket_quotes as websocket_quotes_router
...
app.include_router(websocket_quotes_router.router, prefix="/api", tags=["websocket-quotes"])
...
async def root():
    """根路径，返回API信息"""
    print("🏠 根路径被访问")
    return {"name": "TradingAgents-CN API", "version": get_version(), "status": "running", "docs_url": "/docs" if settings.DEBUG else None}
...
if __name__ == "__main__":
    uvicorn.run(
        ...
        reload_excludes=["__pycache__", "*.pyc", "*.pyo", "*.pyd", ".git", ".pytest_cache", "*.log", "*.tmp"] if settings.DEBUG else None,
