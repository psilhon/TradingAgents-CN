...
async def _get_or_create_account(user_id: str) -> Dict[str, Any]:
    ...
    if not acc:
        ...
        acc = {
            "user_id": user_id,
            # 多货币现金账户
            "cash": {"CNY": INITIAL_CASH_BY_MARKET["CNY"], "HKD": INITIAL_CASH_BY_MARKET["HKD"], "USD": INITIAL_CASH_BY_MARKET["USD"]},
            # 多货币已实现盈亏
...
async def _get_last_price(code: str, market: str) -> tuple[Optional[float], Optional[str]]:
    ...
    OpenSpec change 2026-05-08-realtime-trading-data-flow: hot-path 必须透出
    `last_price_as_of`，让前端 / agent 知道数据新鲜度。
    ...
    if market == "CN":
        ...
        result = await reader.read_quotes([code])
        ...
        db = get_mongo_db()
        basic_info = await db["stock_basic_info"].find_one({"$or": [{"code": code}, {"symbol": code}]}, {"_id": 0, "current_price": 1})
        ...
        try:
            ...
            if quote:
                ...
                if price and float(price) > 0:
                    ...
                    # ForeignStockService 返回的 quote 含 updated_at（datetime）；尝试透出
                    ts_raw = quote.get("updated_at")
...
async def get_account(current_user: dict = Depends(get_current_user)):
    ...
    # 顶层 as_of_ts = min(positions.last_price_as_of)，反映"最 stale 的那条"；
    # 全缺漏时为 None（OpenSpec realtime-trading-data-flow Req 2）
...
async def place_order(payload: PlaceOrderRequest, current_user: dict = Depends(get_current_user)):
    ...
    if side == "buy":
        ...
        if available_cash < total_cost:
            raise HTTPException(status_code=400, detail=f"可用{currency}不足：需要 {total_cost:.2f}，可用 {available_cash:.2f}")
        ...
        if not pos:
            ...
            await db["paper_positions"].update_one(
                {"_id": pos["_id"]},
                {"$set": {"quantity": new_qty, "available_qty": new_available, "avg_cost": new_avg, "updated_at": now_iso}},
            )
        ...
        await db["paper_accounts"].update_one(
            {"user_id": current_user["id"]},
            {"$inc": {f"cash.{currency}": net_proceeds, f"realized_pnl.{currency}": realized_pnl_delta}, "$set": {"updated_at": now_iso}},
        )
        ...
        if new_qty == 0:
            ...
            await db["paper_positions"].update_one(
                {"_id": pos["_id"]}, {"$set": {"quantity": new_qty, "available_qty": new_available, "updated_at": now_iso}}
            )
...
async def list_positions(current_user: dict = Depends(get_current_user)):
    ...
    last_price_timestamps: List[str] = []
    for p in items:
        code = p.get("code")
        market = p.get("market", "CN")
        currency = p.get("currency", "CNY")
    ...
    as_of_ts = min(last_price_timestamps) if last_price_timestamps else None
    return ok({"items": enriched, "as_of_ts": as_of_ts})
...
async def list_snapshots(
    ...
    items = await get_paper_snapshot_service().get_snapshots(current_user["id"], days=days)
