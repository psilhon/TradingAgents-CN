...
class AKShareSyncService:
    ...
    async def sync_stock_basic_info(self, force_update: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(0, len(stock_list), self.batch_size):
                ...
                logger.info(
                    f"📈 基础信息同步进度: {progress}/{len(stock_list)} (成功: {stats['success_count']}, 错误: {stats['error_count']})"
                )
                ...
                if basic_info:
                    ...
                    try:
                        ...
                        batch_stats["errors"].append(
                            {"code": code, "error": f"数据库更新失败: {str(e)}", "context": "update_stock_basic_info"}
                        )
                else:
                    batch_stats["error_count"] += 1
                    batch_stats["errors"].append({"code": code, "error": "获取基础信息失败", "context": "get_stock_basic_info"})

            except Exception as e:
                batch_stats["error_count"] += 1
                batch_stats["errors"].append(
                    {"code": stock_info.get("code", "unknown"), "error": str(e), "context": "_process_basic_info_batch"}
                )
        ...
        try:
            ...
            if len(symbols) == 1:
                ...
                if success:
                    ...
                    stats["error_count"] = 1
                    stats["errors"].append({"code": symbol, "error": "获取行情失败", "context": "sync_realtime_quotes_single"})
                ...
                if not quotes_map:
                    ...
                    for i in range(0, len(symbols), self.batch_size):
                        ...
                        logger.info(
                            f"📈 行情同步进度: {progress}/{len(symbols)} (成功: {stats['success_count']}, 错误: {stats['error_count']})"
                        )
                    ...
                    for i in range(0, len(symbols), self.batch_size):
                        ...
                        for symbol in batch:
                            try:
                                ...
                                if quotes:
                                    ...
                                    stats["error_count"] += 1
                                    stats["errors"].append({"code": symbol, "error": "未找到行情数据", "context": "sync_realtime_quotes"})
                            except Exception as e:
                                stats["error_count"] += 1
                        ...
                        progress = min(i + self.batch_size, len(symbols))
                        logger.info(
                            f"📈 行情保存进度: {progress}/{len(symbols)} (成功: {stats['success_count']}, 错误: {stats['error_count']})"
                        )
            ...
            for symbol in batch:
                try:
                    ...
                    if quotes:
                        ...
                        batch_stats["error_count"] += 1
                        batch_stats["errors"].append({"code": symbol, "error": "未找到行情数据", "context": "_process_quotes_batch"})
                except Exception as e:
                    batch_stats["error_count"] += 1
                    batch_stats["errors"].append({"code": symbol, "error": str(e), "context": "_process_quotes_batch"})
        ...
        for symbol in batch:
            try:
                ...
                if success:
                    ...
                    batch_stats["error_count"] += 1
                    batch_stats["errors"].append({"code": symbol, "error": "获取行情数据失败", "context": "_process_quotes_batch_fallback"})
                ...
                batch_stats["error_count"] += 1
                batch_stats["errors"].append({"code": symbol, "error": str(e), "context": "_process_quotes_batch_fallback"})
    ...
    async def sync_historical_data(
        self, start_date: str = None, end_date: str = None, symbols: List[str] = None, incremental: bool = True, period: str = "daily"
    ) -> Dict[str, Any]:
        """
        同步历史数据
        ...
        try:
            ...
            for i in range(0, len(symbols), self.batch_size):
                ...
                progress = min(i + self.batch_size, len(symbols))
                logger.info(
                    f"📈 历史数据同步进度: {progress}/{len(symbols)} (成功: {stats['success_count']}, 记录: {stats['total_records']})"
                )
    ...
    async def _process_historical_batch(
        self, batch: List[str], start_date: str, end_date: str, period: str = "daily", incremental: bool = False
    ) -> Dict[str, Any]:
        """处理历史数据批次"""
        ...
        for symbol in batch:
            try:
                ...
                if hist_data is not None and not hist_data.empty:
                    ...
                    batch_stats["error_count"] += 1
                    batch_stats["errors"].append({"code": symbol, "error": "历史数据为空", "context": "_process_historical_batch"})
    ...
    async def sync_financial_data(
        ...
        Args:
            symbols: 指定股票代码列表
            limit: 获取财报期数（与 tushare_sync_service 鸭子接口对齐）.
                AKShare provider 一次返回全部历史期数，本服务当前不强制裁剪 ——
                参数接受但内部不传给 provider；保留 default=20 仅为 caller
                兼容性（stock_validator 等会传 limit=20）.
            job_id: 进度跟踪 ID（与 tushare 对齐，akshare 不使用）.
        ...
        try:
            ...
            for i in range(0, len(symbols), self.batch_size):
                ...
                progress = min(i + self.batch_size, len(symbols))
                logger.info(
                    f"📈 财务数据同步进度: {progress}/{len(symbols)} (成功: {stats['success_count']}, 错误: {stats['error_count']})"
                )
            ...
            try:
                ...
                if financial_data:
                    ...
                    if success:
                        ...
                        batch_stats["error_count"] += 1
                        batch_stats["errors"].append({"code": symbol, "error": "财务数据保存失败", "context": "_process_financial_batch"})
                else:
                    batch_stats["error_count"] += 1
                    batch_stats["errors"].append({"code": symbol, "error": "财务数据为空", "context": "_process_financial_batch"})

            except Exception as e:
                batch_stats["error_count"] += 1
                batch_stats["errors"].append({"code": symbol, "error": str(e), "context": "_process_financial_batch"})
    ...
    async def _save_financial_data(self, symbol: str, financial_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            saved_count = await financial_service.save_financial_data(
                symbol=symbol, financial_data=financial_data, data_source="akshare", market="CN", report_type="quarterly"
            )
    ...
    async def run_status_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            status_result = {"provider_connected": provider_connected, "collections": collections_status, "status_time": datetime.utcnow()}
    ...
    async def _get_favorite_stocks(self) -> List[str]:
        ...
        try:
            ...
            users_cursor = self.db.users.find(
                {"favorite_stocks": {"$exists": True, "$ne": []}}, {"favorite_stocks.stock_code": 1, "_id": 0}
            )
    ...
    async def sync_news_data(
        self, symbols: List[str] = None, max_news_per_stock: int = 20, force_update: bool = False, favorites_only: bool = True
    ) -> Dict[str, Any]:
        """
        同步新闻数据
        ...
        try:
            ...
            for i in range(0, len(symbols), self.batch_size):
                ...
                # 进度日志
                progress = min(i + self.batch_size, len(symbols))
                logger.info(f"📈 新闻同步进度: {progress}/{len(symbols)} (成功: {stats['success_count']}, 新闻: {stats['news_count']})")
    ...
    async def _process_news_batch(self, batch: List[str], max_news_per_stock: int) -> Dict[str, Any]:
        ...
        for symbol in batch:
            try:
                ...
                if news_data:
                    # 保存新闻数据
                    saved_count = await self.news_service.save_news_data(news_data=news_data, data_source="akshare", market="CN")
